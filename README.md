...
По умолчанию UI читает исходные файлы из `data/`, а готовые результаты из `output/`:
...
| `node_metrics.csv` | `gid, depth, is_seed, truncated_by_depth` для оговорок сбора; остальные метрики разрешены |
| `edges.csv` | `src, dst, sum_kzt, n_tx`; также поддержан формат `source, target, amount` |
...
Приоритет отображается с тремя знаками после запятой, как в актуальной `analytics`.
Метаданные seed/depth сначала берутся из `node_metrics.csv`, затем дополняются
из `nodes.parquet`. Они не меняют роль, оценку или объяснение из `nodes_roles.csv`.
...
Для полного набора тестов: `python -m pip install -r requirements-test.txt`.
Интеграционные тесты пишут результаты во временную папку и не меняют рабочий `output/`.

## Ветки и структура

Рабочие ветки: `dev/ui`, `analytics`, `dev/data-metrics`. `main` остаётся общей
веткой репозитория. Устаревшая `graph-metrics` удалена; её история уже входит в
`dev/data-metrics`. В этой поставке рабочие ветки не сливались: UI адаптирован
к их CSV-контракту. [Передача результатов между ветками](docs/integration.md).

- `ui/`, `.streamlit/`, `requirements-ui.txt` — интерфейс и его запуск.
- `data/` — единственный экземпляр исходного датасета.
- `mock/` — синтетические данные для автономного демо и UI-тестов.
- `output/` — локальные результаты команды; CSV не хранятся в Git.
- `src/`, `tests/` — существующий код и проверки; UI не вызывает расчёты из `src/`.
- `docs/` — [задание](docs/task.pdf), [контракт данных](docs/data-contract.md), демо и скриншоты.

ZIP-дубликаты, `__MACOSX`, старый starter и байткод исключены из проекта.
При чистом клонировании без `output/*.csv` автоматически открывается маркированный mock.
...
Результаты веток команды подключаются через `output/`. UI показывает переданные
