...
- **Дизайн-кейс:** явно маркированный вымышленный пример, включается отдельно; не сдаётся как аналитика.

## Что умеет UI

- Поиск любого GID, быстрый переход к TOP-20, переход к контрагенту.
...
разделение дизайна и реальных данных, подключение CSV и восстановление порядка TOP.
...
В текущей версии репозитория аналитический код — исходный `starter.py`, который
пишет пустые роли. Финальный пайплайн, формальные критерии ролей, рассчитанные
2 248 строк и три итоговые выгрузки должны быть подключены командой перед сдачей.
Этот README описывает воспроизводимый запуск **UI**, а не обещает готовый полный анализ.
